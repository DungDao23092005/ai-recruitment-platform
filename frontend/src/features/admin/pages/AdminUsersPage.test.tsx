...
function makeList(
  items: AdminUser[] = [candidate, recruiter, locked],
  total = items.length,
): AdminUserList {
...
describe('AdminUsersPage', () => {
  ...
  it('disables the deactivate action for a locked account', async () => {
    renderPage()
    ...
    expect(
      screen.getByRole('button', { name: 'Khóa tài khoản locked@example.com' }),
    ).toBeDisabled()
  })

  it('deactivates a user after confirmation and reloads the list', async () => {
    mockedDeactivateAdminUser.mockResolvedValue(makeUser({ is_deleted: true }))
    ...
    expect(
      screen.getByRole('dialog', { name: 'Khóa tài khoản người dùng' }),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/không thể đăng nhập/i),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/vẫn được lưu giữ và không bị xóa/i),
    ).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: 'Khóa tài khoản' }))

    await waitFor(() => {
      expect(mockedDeactivateAdminUser).toHaveBeenCalledWith('user-1')
    })
